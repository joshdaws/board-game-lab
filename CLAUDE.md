...
Board Game Lab is a knowledge base for AI-assisted board game design. It studies the top 100 ranked games on BoardGameGeek to extract design patterns, mechanic taxonomies, and reusable insights. There is no application code, build system, or tests — this is a structured Markdown + PDF research repository.

## Repository Structure

- `games/` — One Markdown file per game, numbered by BGG rank (01-100). `index.md` tracks the research status of each game.
- `rulebooks/` — Source PDF rulebooks for games #1-25 (gitignored, not committed). Games #26-100 were enriched from designer knowledge without rulebook PDFs.
- `mechanics/` — `index.md` has mechanic frequency counts across all 100 games (113 unique mechanics); `categories.md` has theme/category frequency (57 unique categories).
- `knowledge/` — `design-patterns.md` synthesizes cross-cutting insights from the full 100-game dataset, including weight distribution, 15 core design insights, 8 design archetypes, and mechanic combination analysis.

## Game File Format

All 100 game files are enriched. Each file contains:
