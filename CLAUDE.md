...
Board Game Lab is a knowledge base for AI-assisted board game design. It studies the top 125 ranked games on BoardGameGeek to extract design patterns, mechanic taxonomies, and reusable insights. There is no application code, build system, or tests — this is a structured Markdown + PDF research repository.

## Repository Structure

- `games/` — One Markdown file per game, numbered by BGG rank (01-125). `index.md` tracks the research status of each game.
- `rulebooks/` — Source PDF rulebooks (gitignored, not committed). Games #1-25 have direct-download PDFs; most games #26-125 were enriched from designer knowledge without rulebook PDFs.
- `mechanics/` — `index.md` has mechanic frequency counts across all 125 games; `categories.md` has theme/category frequency.
- `knowledge/` — `design-patterns.md` synthesizes cross-cutting insights from the full 125-game dataset, including weight distribution, 15 core design insights, 8 design archetypes, and mechanic combination analysis.

## Game File Format

All 125 game files are enriched. Each file contains:
...
The skill's `references/` directory contains a game index (all 125 games with archetype classifications), plus copies of the design patterns, mechanics taxonomy, and categories taxonomy. The `assets/` directory has a game design document template.
